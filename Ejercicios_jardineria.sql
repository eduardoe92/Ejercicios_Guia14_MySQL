...
FROM
    empleado
WHERE
    puesto NOT REGEXP 'representante ventas';
...
cuenta que deberá eliminar aquellos códigos de cliente que aparezcan repetidos.
...
SELECT DISTINCT
    codigo_cliente, DATE_FORMAT(fecha_pago, '%Y')
FROM
    pago
WHERE
...
FROM
    pedido
WHERE
    fecha_entrega <= ADDDATE(fecha_esperada,
        INTERVAL - 2 DAY);
...
WHERE
    gama = 'Ornamentales'
        AND cantidad_en_stock > 100
ORDER BY precio_proveedor DESC;
...
SELECT 
    nombre_cliente, nombre, apellido1, puesto
FROM
    cliente c
        INNER JOIN
...
SELECT 
    nombre_cliente, nombre, apellido1, puesto, fecha_pago
FROM
    cliente c
        INNER JOIN
    empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
SELECT 
    nombre_cliente, nombre, apellido1, puesto, o.ciudad
FROM
    cliente c
        INNER JOIN
    empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
FROM
    cliente c
        INNER JOIN
    oficina o ON c.region = o.region
WHERE
...
SELECT 
    nombre_cliente, nombre, apellido1, o.ciudad
FROM
    cliente c
        INNER JOIN
    empleado e ON c.codigo_empleado_rep_ventas = e.codigo_empleado
...
SELECT 
    e1.nombre, e1.apellido1, e2.nombre AS nombre_jefe
FROM
    empleado e1
        INNER JOIN
    empleado e2 ON e1.codigo_jefe = e2.codigo_empleado;

#9. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
SELECT 
    nombre_cliente, (fecha_entrega - fecha_esperada) dias_demora
FROM
    pedido p
        INNER JOIN
...
SELECT 
    g.gama, COUNT(g.gama) AS Total_Vendidos
FROM
    gama_producto g
        INNER JOIN
    producto pr ON pr.gama = g.gama
        INNER JOIN
    detalle_pedido d ON pr.codigo_producto = d.codigo_producto
GROUP BY g.gama;

SELECT 
    c.nombre_cliente, g.gama, COUNT(g.gama) AS Cantidad
FROM
    cliente c
        INNER JOIN
...
/*17. La misma información que en la pregunta anterior, pero agrupada por código 
