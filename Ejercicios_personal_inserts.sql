...
SELECT 
    nombre, (comision_emp + sal_emp + 500) AS Total
FROM
    empleados
WHERE
    id_depto = 3000
...
SELECT 
    nombre_depto, ROUND(AVG(sal_emp))
FROM
    empleados,
    departamentos
...
SELECT 
    COUNT(nombre_depto), nombre_depto
FROM
    departamentos
GROUP BY nombre_depto
HAVING COUNT(nombre_depto) = 0
LIMIT 0 , 1000;
...
SELECT 
    ...
    d.nombre_jefe_depto AS jefe_departamento
FROM
    empleados e
        JOIN
